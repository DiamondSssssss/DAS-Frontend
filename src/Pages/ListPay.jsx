...
import { fetchListPay } from "redux/listPaySlice";
...
export const ListPay = () => {
  ...
  const { entities, error, loading } = useSelector( ( state ) => state.listPay );
  ...
  const dataButton = ( questionSetId, name ) => [
    {
      ...
      url: eye,
      event: () => {
        navigate( PATH.LISTPAY + `${ "/" + questionSetId }`, {
  ...
  useEffect( () => {
    dispatch( fetchListPay( { token, params } ) );
  }, [] );
